...
class PCIMutation:
    ...
    def mutate_pci(self):
        """Applies Parent Cast Insertion (PCI) mutations."""
        print("Applying PCI mutations...")
                ...
                for node in ast.walk(mutated_tree):
                    if isinstance(node, ast.Assign):
                        if isinstance(node.value, ast.Call):
                            if isinstance(node.value.func, ast.Name) and node.value.func.id == child_class:
                                # Found object creation with child class
                                lineno = node.lineno

                                # Create a new Assign node for the type cast
                                new_node = ast.Assign(
                                    targets=[copy.deepcopy(node.targets)],
                                    value=ast.Call(
                                        func=ast.Name(id=parent_class, ctx=ast.Load()),
                                        args=[copy.deepcopy(node.targets)],
                                        keywords=[]
                                    )
                                )
                                ast.fix_missing_locations(new_node)

                                # Insert the type cast after the original assignment
                                new_line = astunparse.unparse(new_node).strip() + "\n"
                                mutated_code_lines.insert(lineno, new_line)

                                # Save mutated code
                                mutated_code = "".join(mutated_code_lines)
                                self.save_code(mutated_code, "PCI", [child_class, parent_class])

                                # Restore for the next mutation within the same child class
                                mutated_code_lines = list(original_code_lines)
                                mutated_tree = copy.deepcopy(tree)
